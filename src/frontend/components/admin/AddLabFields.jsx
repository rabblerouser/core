...
const EditLabFields = ({ invalidFields, onChange, formValues}) => {
    ...
    return (
        <section>
            ...
            <input type="text" defaultValue={formValues.contact} onChange={onChange('contact')} id="contact" className="contact"/>
            <FormFieldLabel fieldName="notes" hasError={isValidationError('notes')} />
            <input type="text" defaultValue={formValues.notes} onChange={onChange('notes')} id="notes" className="notes"/>
        </section>
    )
}
