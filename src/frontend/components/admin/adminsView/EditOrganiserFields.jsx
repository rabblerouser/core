...
const EditOrganiserFields = ({ invalidFields, onChange, formValues}) => {
    ...
    return (
        <section>
            ...
            <input type="password" placeholder='••••••••••••' onChange={onChange('password')} id="password" className="password"/>
            {
                formValues.password ? <FormFieldLabel fieldName="confirmedPassword" hasError={isValidationError('confirmedPassword')} />
                                    : ''
            }
            {
                formValues.password ? <input type="password" onChange={onChange('confirmedPassword')} id="confirmedPassword" className="password"/> : ''
            }
        </section>
    )
}
