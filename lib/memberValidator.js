...
var isPhoneNumber = (input) => {
  ...
  if(number.indexOf('+61') === -1) {
    let internationalTest = /^\+(\d){3,15}$/.test(number);
    let localTest = /^0[23478][0-9]{8}$/.test(number);
      return internationalTest || localTest;
  }
  return /^\+61[23478][0-9]{8}$/.test(number);
