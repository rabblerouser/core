...
var logUpdateInvoiceEvent = (invoice) => {
    winstonLogger.info('[update-invoice-event]', {invoice: invoice.dataValues });
};

var logCreateEmptyInvoiceEvent = (invoice) => {
    winstonLogger.info('[create-emtpy-invoice-event]', { invoice: invoice.dataValues});
};
...
var invalidPaypalIpnRequest = (invoiceId, txnId, paymentStatus, receiverEmail) => {
    winstonLogger.info('[paypal-invalid-ipn-request]', { invoiceId: invoiceId, txnId: txnId, paymentStatus: paymentStatus, receiverEmail: receiverEmail});
};
