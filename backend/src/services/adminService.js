...
const setupNewAdmin = newAdmin => Object.assign({}, newAdmin, { id: createHash(), memberSince: moment() });

const create = newAdmin =>
  Q.all(setupNewAdmin(newAdmin))
    .then(save)
    .tap(logEvent)
    .then(transformAdmin)
...
const updateAdmin = newValues =>
  AdminUser.findOne({ where: { id: newValues.id } })
