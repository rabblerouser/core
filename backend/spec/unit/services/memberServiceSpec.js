...
function fakeMembersList(membersQty) {
  ...
  times(membersQty, () => {
    const dbResponse = fakeResultFromDbWhenSavingMember();
...
describe('memberService', () => {
  describe('createMember', () => {
    ...
    beforeEach(() => {
      ...
      sinon.stub(branchService, 'findById').withArgs('some-branch-id-1').returns(branchPromise.promise);
    });
    ...
    it('creates a new member with same residential and postal address', done => {
      ...
      const input = fakeNewMember(fakeResidentialAddress(), fakeResidentialAddress());
    ...
    describe('things went bad', () => {
      it('handles error when retrieving the branch', done => {
        ...
        const input = fakeNewMember(null, null);
      ...
      it('handles db errors when saving the residential address', done => {
        ...
        const input = fakeNewMember(fakeResidentialAddress(), fakePostalAddress());
  ...
  describe('list', () => {
    ...
    it('returns a list of members for a specific branch', done => {
      ...
      memberService.list('some-branch-id-1')
            .then(result => {
              const sampleMember = sample(result);
